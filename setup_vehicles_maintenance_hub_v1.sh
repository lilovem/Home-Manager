wq#!/bin/bash
set -e
echo "== שלב 1/3: הוספת מחרוזות (AppStrings) =="
python3 - << 'PYEOF'
import json, re
path = 'lib/app/config/app_strings.dart'
entries = [
    ("maintenanceLayoutPickerTitle", "עיצוב תצוגת הטיפולים"),
    ("maintenanceLayoutIconsOption", "שורת אייקונים"),
    ("maintenanceLayoutHubOption", "עיגול מרכזי"),
]
...
with open(path, 'w', encoding='utf-8') as f:
    f.write(content)
for k in added: print(f'  {k} נוסף.')
for k in skipped: print(f'  {k} כבר קיים - מדלג.')
PYEOF

echo "== שלב 2/3: עדכון vehicle_detail_screen.dart =="
...
class VehicleDetailScreen extends ConsumerWidget {
  ...
  final GlobalKey _serviceHistoryKey = GlobalKey();
  ...
  void _scrollToHistory(BuildContext context) {
    final historyContext = _serviceHistoryKey.currentContext;
  ...
  void _showMaintenanceLayoutPicker(BuildContext context, String currentLayout) {
        ...
        child: Column(
          ...
          children: [
            Padding(
              ...
              child: Text(AppStrings.maintenanceLayoutPickerTitle, style: AppTextStyles.heading2),
            ),
            ListTile(
              leading: const Icon(Icons.view_agenda_outlined),
              title: const Text(AppStrings.maintenanceLayoutIconsOption),
              trailing: currentLayout == 'icons'
                  ? const Icon(Icons.check, color: AppColors.primary)
                  : null,
              onTap: () {
                setVehiclesMaintenanceLayout(householdId, 'icons');
                Navigator.of(sheetContext).pop();
              },
            ),
            ListTile(
              leading: const Icon(Icons.donut_large_outlined),
              title: const Text(AppStrings.maintenanceLayoutHubOption),
              trailing: currentLayout == 'hub'
                  ? const Icon(Icons.check, color: AppColors.primary)
                  : null,
              onTap: () {
                setVehiclesMaintenanceLayout(householdId, 'hub');
                Navigator.of(sheetContext).pop();
              },
            ),
          ],
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final maintenanceLayout = ref.watch(vehiclesMaintenanceLayoutProvider(householdId)).value ?? 'icons';
              ...
              SliverAppBar(
                ...
                actions: [
                  IconButton(
                    ...
                    tooltip: AppStrings.maintenanceLayoutPickerTitle,
                    onPressed: () => _showMaintenanceLayoutPicker(context, maintenanceLayout),
                  ),
                  IconButton(
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      maintenanceLayout == 'hub'
                          ? _MaintenanceHubSection(
                              householdId: householdId,
                              vehicle: vehicle,
                              onEditInterval: (key, interval) =>
                                  _editInterval(context, ref, vehicle, key, interval),
                              onAddService: () => Navigator.of(context).push(
                                MaterialPageRoute(
...
/// עיצוב חלופי למקטע הטיפולים: עיגול מרכזי גדול, מחולק לשניים -
/// חצי ימני פותח את היסטוריית הטיפולים (גולל אליה למטה), חצי שמאלי
/// פותח הוספת טיפול חדש - עם 4 אייקוני סוגי הטיפולים "מקיפים" אותו
/// מארבעת הצדדים. קומפקטי וממורכז, לא תופס רוחב מסך מלא. הבחירה בין
/// העיצוב הזה לעיצוב "שורת האייקונים" (_MaintenanceSection) נשמרת
/// פר-household דרך vehiclesMaintenanceLayoutProvider.
class _MaintenanceHubSection extends ConsumerWidget {
  final String householdId;
  final Vehicle vehicle;
  final void Function(String templateKey, int currentInterval) onEditInterval;
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 12),
              Text(kMaintenanceTemplateDescriptions[key] ?? '', style: AppTextStyles.bodySecondary),
              const SizedBox(height: 20),
              OutlinedButton.icon(
                onPressed: () {
                  Navigator.of(sheetContext).pop();
  ...
  Widget _orbitIcon(BuildContext context, String key, Map<String, dynamic> data) {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return recordsAsync.when(
      ...
      error: (e, st) => const SizedBox.shrink(),
      data: (records) {
        final perKey = <String, Map<String, dynamic>>{};
        for (final key in kTrackedMaintenanceKeys) {
          final name = kMaintenanceTemplateNames[key] ?? key;
          final interval =
          ...
          perKey[key] = {
            'name': name,
            'icon': _icons[key] ?? Icons.build_outlined,
            'color': color,
            'remaining': remaining,
            'interval': interval,
          };
        ...
        final keys = kTrackedMaintenanceKeys;
          ...
          child: SizedBox(
            ...
            child: Stack(
              ...
              children: [
                ...
                if (keys.isNotEmpty)
                  Positioned(top: 0, left: 0, child: _orbitIcon(context, keys[0], perKey[keys[0]]!)),
                if (keys.length > 1)
                  Positioned(top: 0, right: 0, child: _orbitIcon(context, keys[1], perKey[keys[1]]!)),
                if (keys.length > 2)
                  Positioned(bottom: 0, left: 0, child: _orbitIcon(context, keys[2], perKey[keys[2]]!)),
                if (keys.length > 3)
                  Positioned(bottom: 0, right: 0, child: _orbitIcon(context, keys[3], perKey[keys[3]]!)),
              ],
            ),
          ),
        );
...
echo "== שלב 3/3: עדכון vehicles_background_provider.dart =="
cat > lib/features/vehicles/vehicles_background_provider.dart << 'DARTEOF'
...
/// בחירת עיצוב מקטע הטיפולים במסך פרטי הרכב - 'icons' (שורת
/// אייקונים קטנים, ברירת המחדל) או 'hub' (עיגול מרכזי מחולק עם
/// אייקוני הטיפולים מקיפים אותו). שמור פר-household, כמו הרקע.
final vehiclesMaintenanceLayoutProvider =
    StreamProvider.family<String, String>((ref, householdId) {
  final firestore = ref.watch(_vehiclesFirestoreForBackgroundProvider);
  return firestore.collection('households').doc(householdId).snapshots().map(
        (doc) => (doc.data()?['vehiclesMaintenanceLayout'] as String?) ?? 'icons',
      );
});

Future<void> setVehiclesMaintenanceLayout(String householdId, String layoutId) {
  return FirebaseFirestore.instance
      .collection('households')
      .doc(householdId)
      .update({'vehiclesMaintenanceLayout': layoutId});
}
DARTEOF
