#!/bin/bash
set -e
echo "== שלב 1: הוספת מחרוזות (AppStrings) =="
python3 - << 'PYEOF'
import json, re
path = 'lib/app/config/app_strings.dart'
entries = [
    ("maintenanceSettingsTitle", "הגדרות טווחי טיפולים"),
    ("nextServiceInLabel", "הטיפול הבא בעוד"),
    ("muteMaintenanceTooltip", "השתק התראות לטיפול זה"),
    ("unmuteMaintenanceTooltip", "הפעל התראות לטיפול זה"),
]
...
echo "== שלב 2: עדכון lib/models/vehicle_model.dart =="
mkdir -p lib/models
...
class Vehicle {
  ...
  /// מפתחות סוגי טיפול (מתוך kTrackedMaintenanceKeys) שההתראה
  /// החזותית עליהם "מושתקת" - כלומר גם אם הטיפול דחוף/באיחור, לא
  /// יוצג עליו סימון אדום/דחיפות. אין כאן שום דבר שקשור להתראות
  /// Push אמיתיות (לא הוקמה תשתית כזו בכוונה, כדי לא לחייב תוכנית
  /// Blaze בתשלום) - זה סימון חזותי בתוך האפליקציה בלבד.
  final List<String> mutedMaintenanceKeys;
  ...
  const Vehicle({
    ...
    this.photoDataUrl,
    this.mutedMaintenanceKeys = const [],
...
echo "== שלב 3: עדכון lib/services/firebase/vehicles_service.dart =="
...
class VehiclesService {
  ...
  /// משתיק/מפעיל מחדש את הסימון החזותי (דחיפות/אדום) עבור סוג
  /// טיפול מסוים ברכב הזה. זה *לא* התראת Push אמיתית - רק דגל
  /// שמונע מהאייקון/כרטיס של הטיפול הזה להיראות "דחוף" גם אם הוא
  /// בפועל באיחור, כדי שלא יטרידו הודעות/צבעים על טיפול שהמשתמשת
  /// בחרה להתעלם ממנו כרגע.
  Future<void> setMaintenanceMuted({
    required String householdId,
    required String vehicleId,
    required String templateKey,
    required bool muted,
  }) {
    final ref = _vehiclesCollection(householdId).doc(vehicleId);
    return ref.update({
      'mutedMaintenanceKeys': muted
          ? FieldValue.arrayUnion([templateKey])
          : FieldValue.arrayRemove([templateKey]),
    });
  }
...
echo "== שלב 4: עדכון lib/repositories/vehicles_repository.dart =="
...
class VehiclesRepository {
  ...
  Future<void> setMaintenanceMuted({
    ...
    return _service.setMaintenanceMuted(
      householdId: householdId,
      vehicleId: vehicleId,
      templateKey: templateKey,
      muted: muted,
...
echo "== שלב 5: עדכון lib/features/vehicles/vehicle_detail_screen.dart =="
...
class VehicleDetailScreen extends ConsumerWidget {
  ...
  /// מקטע הטיפולים. מרכזת עבור כל סוג טיפול: את המרווח הנוכחי (עם
  /// אפשרות עריכה), ואת מתג ההשתקה (מסמנת חזותית בלבד - לא Push).
                    ...
                    children: [
                      ...
                      ...kTrackedMaintenanceKeys.map((key) {
                        ...
                        final interval = vehicle.maintenanceIntervals[key] ??
                            ...
                            10000;
                        final muted = vehicle.mutedMaintenanceKeys.contains(key);
                          ...
                          trailing: Row(
                            ...
                            children: [
                              ...
                              IconButton(
                                ...
                                tooltip: muted
                                    ? AppStrings.unmuteMaintenanceTooltip
                                    : AppStrings.muteMaintenanceTooltip,
                                onPressed: () =>
                                    ref.read(vehiclesRepositoryProvider).setMaintenanceMuted(
                                          householdId: householdId,
                                          vehicleId: vehicle.id,
                                          templateKey: key,
                                          muted: !muted,
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          IconButton(
                            ...
                            tooltip: AppStrings.maintenanceSettingsTitle,
                            onPressed: () => _showMaintenanceSettingsSheet(context, ref),
                          ),
                        ],
                      ),
                      const SizedBox(height: 8),
    ...
    return recordsAsync.when(
      ...
      data: (records) {
        final tiles = kTrackedMaintenanceKeys.map((key) {
          ...
          final isMuted = vehicle.mutedMaintenanceKeys.contains(key);
          ...
          final isUrgent = !isMuted && remaining < 0;
...
echo "== שלב 6: עדכון lib/features/vehicles/vehicles_list_screen.dart =="
mkdir -p lib/features/vehicles
cat > lib/features/vehicles/vehicles_list_screen.dart << 'DARTEOF'
import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import '../../app/config/app_colors.dart';
import '../../app/config/app_strings.dart';
