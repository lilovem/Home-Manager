#!/bin/bash
set -e
echo "=== מעצב מחדש את מקטע הטיפולים - רשת קומפקטית ומודרנית ==="

mkdir -p lib/features/vehicles

echo "כותב lib/features/vehicles/vehicle_detail_screen.dart..."
cat > lib/features/vehicles/vehicle_detail_screen.dart << 'GRID_REDESIGN_EOF'
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          ...
                          InkWell(
                            borderRadius: BorderRadius.circular(14),
                            onTap: () => onEditInterval(key, interval),
                            child: const Padding(
                              padding: EdgeInsets.all(4),
                              child: Icon(Icons.tune, size: 15, color: AppColors.textSecondary),
                            ),
                          ),
                        ],
                      ),
