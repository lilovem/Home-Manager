#!/bin/bash
set -e
mkdir -p lib/core/utils lib/features/shopping lib/features/bills lib/services/firebase lib/repositories
...
class WaterAndTaxScreen extends ConsumerWidget {
  ...
  Future<void> _reset(WidgetRef ref) {
    return ref.read(billsRepositoryProvider).resetWaterTaxChoice(householdId);
  }
...
HMEOF
echo 'DONE - everything combined: water/tax reset button, centered titles, dialog hint, and real product emoji!'
