#!/bin/bash
set -e
mkdir -p lib/models lib/services/firebase lib/repositories lib/features/bills
...
Future<void> openPaymentLink(String url) {
  // web_only WindowName='_blank' חיוני - בלעדיו הדפדפן מנווט את
  // אותו חלון (מאפס את אפליקציית ה-Flutter) במקום לפתוח כרטיסייה
  // חדשה.
  return launchUrl(
    Uri.parse(url),
...
class _BillPeriodEditSheetState extends ConsumerState<BillPeriodEditSheet> {
  ...
  Future<void> _openPaymentApp() async {
    final isBit = _paymentMethod == AppStrings.paymentMethodBit;
    final settings = ref.read(billLinkSettingsProvider(widget.householdId)).value;
    var url = isBit ? settings?.bitUrl : settings?.payboxUrl;

    if (url == null || url.isEmpty) {
      url = await _promptForAppUrl(isBit ? AppStrings.paymentMethodBit : AppStrings.paymentMethodPaybox);
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          if (widget.showPaymentMethod &&
              (_paymentMethod == AppStrings.paymentMethodBit ||
                  _paymentMethod == AppStrings.paymentMethodPaybox)) ...[
            const SizedBox(height: 8),
            OutlinedButton.icon(
              onPressed: _openPaymentApp,
              icon: const Icon(Icons.open_in_new, size: 18),
              label: Text('${AppStrings.openAppPrefix} $_paymentMethod'),
            ),
          ],
          const SizedBox(height: 16),
          ElevatedButton(
...
///
/// בבחירת "ביט" או "פייבוקס" כאמצעי תשלום בחלונית העריכה, מוצג
/// כפתור שפותח את האפליקציה/קישור המתאים (ראה BillPeriodEditSheet).
...
HMEOF
echo 'DONE - Bit/PayBox open the actual app, electricity/water/tax have no payment method options!'
